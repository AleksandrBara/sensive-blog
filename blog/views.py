...
def serialize_post(post):
    return {
        ...
        'slug': post.slug,
        'tags': [serialize_tag(tag, post) for tag in post.tags.all()],
...
def index(request):
    ...
    fresh_posts = Post.objects.all().prefetch_related(
        'author',
        'tags',
    ).annotate(
        comments_count=Count('comments'),
    ).order_by('-published_at')[:PAGE_AREA_COUNT] \
    ...
    context = {
        ...
        'page_posts': [
            serialize_post_optimized(post) for post in the_freshest_posts
        ],
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all()[:20].fetch_with_comments_count().fetch_with_tags_count()
